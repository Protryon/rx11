...
use crate::coding::xinput2::{
    self, ChangeMode, DeviceId, DeviceType as XIDeviceType, EventMode, GrabType, HierarchyChangeData, HierarchyChangeType, XIAllowEventsRequest,
    XIChangeHierarchyRequest, XIGetFocusRequest, XIGetFocusResponse, XIGrabDeviceRequest, XIGrabDeviceResponse, XIPassiveGrabDeviceRequest,
    XIPassiveGrabDeviceResponse, XIPassiveUngrabDeviceRequest, XIQueryDeviceRequest, XIQueryDeviceResponse, XISetClientPointerRequest, XISetFocusRequest,
    XIUngrabDeviceRequest,
};
pub use crate::coding::xinput2::{GrabMode, GrabMode22, GrabModifierInfo, GrabStatus, ScrollFlags, ScrollType, TouchMode, ValuatorMode, XIEventMask};
...
pub enum DeviceType<'a> {
    FloatingSlave,
    SlavePointer { master: Device<'a> },
    SlaveKeyboard { master: Device<'a> },
    MasterPointer { paired_keyboard: Device<'a> },
    MasterKeyboard { paired_pointer: Device<'a> },
}

impl<'a> DeviceType<'a> {
    pub(crate) fn from_attachment(type_: XIDeviceType, attachment: Device<'a>) -> Self {
        match type_ {
            XIDeviceType::MasterPointer => DeviceType::MasterPointer {
                paired_keyboard: attachment,
            },
            XIDeviceType::MasterKeyboard => DeviceType::MasterKeyboard {
                paired_pointer: attachment,
            },
            XIDeviceType::SlavePointer => DeviceType::SlavePointer {
                master: attachment,
            },
            XIDeviceType::SlaveKeyboard => DeviceType::SlaveKeyboard {
                master: attachment,
            },
...
pub enum AllowEventMode<'a> {
    ...
    AsyncPair,
    SyncPair,
    AcceptTouch { id: TouchId, grab_window: Window<'a> },
    RejectTouch { id: TouchId, grab_window: Window<'a> },
}
...
impl<'a> Device<'a> {
    pub async fn query(self) -> Result<Vec<DeviceData<'a>>> {
        let reply = send_request_xinput!(
            self.connection,
            XIOpcode::XIQueryDevice,
            XIQueryDeviceResponse,
    ...
    pub async fn grab(
        ...
        let reply = send_request_xinput!(
            self.connection,
            XIOpcode::XIGrabDevice,
            XIGrabDeviceResponse,
            XIGrabDeviceRequest {
    ...
    pub async fn ungrab(self) -> Result<()> {
        send_request_xinput!(
            self.connection,
            XIOpcode::XIUngrabDevice,
            XIUngrabDeviceRequest {
                device: self.id,
            }
        );
    ...
    pub async fn allow_events(self, time: Timestamp, event_mode: AllowEventMode<'_>) -> Result<()> {
        send_request_xinput!(
            self.connection,
            XIOpcode::XIAllowEvents,
            XIAllowEventsRequest {
                ...
                event_mode: match event_mode {
                    ...
                    AllowEventMode::AcceptTouch {
                        ..
                    } => EventMode::AcceptTouch,
                    AllowEventMode::RejectTouch {
                        ..
                    } => EventMode::RejectTouch,
                },
    ...
    pub async fn passive_grab(
        ...
        let reply = send_request_xinput!(
            self.connection,
            XIOpcode::XIPassiveGrabDevice,
            XIPassiveGrabDeviceResponse,
            XIPassiveGrabDeviceRequest {
                ...
                cursor: match &grab {
                    PassiveGrab::Button {
                        cursor,
                        ..
                    } => cursor.map(|x| x.handle).unwrap_or(0),
                    _ => 0,
                },
                device: self.id,
    ...
    pub async fn passive_ungrab(self, window: Window<'_>, grab: PassiveUngrab, modifiers: impl AsRef<[u32]>) -> Result<()> {
        send_request_xinput!(
            self.connection,
            XIOpcode::XIPassiveUngrabDevice,
            XIPassiveUngrabDeviceRequest {
                grab_window: window.handle,
                detail: match &grab {
                    PassiveUngrab::Button {
                        button,
                    } => *button,
                    PassiveUngrab::Keycode {
                        keycode,
                    } => *keycode,
                    _ => 0,
                },
...
impl X11Connection {
    pub async fn xi_change_hierarchy(&self, changes: impl IntoIterator<Item = HierarchyChange<'_>>) -> Result<()> {
        send_request_xinput!(
            self,
            XIOpcode::XIChangeHierarchy,
            XIChangeHierarchyRequest {
                changes: changes
                    .into_iter()
                    .map(|change| match change {
                        HierarchyChange::AddMaster {
                            send_core,
                            enable,
                            name,
                        } => xinput2::HierarchyChange {
                        ...
                        HierarchyChange::RemoveMasterAndFloat {
                            device,
                        } => xinput2::HierarchyChange {
                        ...
                        HierarchyChange::RemoveMasterAndAttach {
                            device,
                            new_pointer_master,
                            new_keyboard_master,
                        } => xinput2::HierarchyChange {
